...
test(`getFirstNEntriesFromPartialCSV`, async () => {
  const csvString = await readFile("lib/stage1/__fixtures__/recruiters/all.csv");
...
test(`getFirstCSVFileRow`, async () => {
  const a = await getFirstCSVFileRow("lib/stage1/__fixtures__/recruiters/all.csv");
  expect(a).toEqual([
    "Date",
    "Company",
    "Method",
  ...
  const b = await getFirstCSVFileRow("lib/stage1/__fixtures__/recruiters/1line.csv");
  expect(b).toEqual(a);

  let didThrow = false;
  try {
    await getFirstCSVFileRow("lib/stage1/__fixtures__/recruiters/fail.csv");
  } catch {
    didThrow = true;
  }
...
test(`getLastCSVFileRow`, async () => {
  const a = await getLastCSVFileRow("lib/stage1/__fixtures__/recruiters/all.csv");
  expect(a).toEqual([
    "2022/01/13",
    "<Our client>",
    "Phone",
  ...
  const b = await getLastCSVFileRow("lib/stage1/__fixtures__/recruiters/1line.csv");
  ...
  let didThrow = false;
  try {
    await getLastCSVFileRow("lib/stage1/__fixtures__/recruiters/fail.csv");
  } catch {
    didThrow = true;
  }
