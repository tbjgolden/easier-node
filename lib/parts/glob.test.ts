...
test("globToRegex", () => {
  ...
  expect(
    globToRegex(`./nothing/much.here`).test(process.cwd() + "/nothing/much.here")
  ).toBe(true);
  ...
  expect(
    globToRegex(`./**/*.{ts,tsx}`).test(process.cwd() + "/wow/foo/bar/test.ts")
  ).toBe(true);
  ...
  expect(globToRegex(`./**/{.keep,.*.tmp}`).test(process.cwd() + "/.keep.temp")).toBe(
    false
  );
  expect(globToRegex(`./**/{.keep,.*.tmp}`).test(process.cwd() + "/.keep.tmp")).toBe(
    true
  );
  expect(globToRegex(`./**/{.keep,.*.tmp}`).test(process.cwd() + "/a/.hi.tmp")).toBe(
    true
  );
});
